...
class TaskbarObject(QWidget):
    ...
    def __init__(
        ...
        if self.pixmap.isNull():
            raise FileNotFoundError(
                f"Could not load object sprite: {image_path}"
            )
    ...
    def keep_behind_tama(self):
        ...
        # Both windows remain in the topmost band,
        # but this window is inserted immediately
        # after (behind) Tama in that band's z-order.
...
class TamaUI(QWidget):
    ...
    def _load(
        ...
        if pixmap.isNull():
            raise FileNotFoundError(
                f"Could not load UI sprite: {path}"
            )
    ...
    def _button_at(
        ...
        # The aligned button art doubles
        # as its exact alpha-aware hit mask.
        for (
            name,
            (
                normal,
    ...
    def spawn_selected_object(self):
        ...
        if (
            self.tama_window is not None
            and self.tama_window.interaction_target == "food"
        ):
            return
        # Only one taskbar object exists at once.
        if (
            self.active_object
            is not None
        ):
        ...
        item = TaskbarObject(
            ...
            y_offset=(
                self.OBJECT_Y_OFFSETS[
                    self.selected_option
                ]
            ),
        )

        item.place_on_taskbar(
        ...
        if self.tama_window is not None:
            object_center_x = (
                item.x()
                + (item.width() // 2)
            )

            self.tama_window.seek_interaction(
                self.selected_option,
                object_center_x,
                self
            )
